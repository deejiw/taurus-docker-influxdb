...
class JMXasDict(JMX):
    ...
    # Added support for GaussianRandomTimer
    def __get_constant_timer(self, element):
        ...
        if hashtree is not None and hashtree.tag == "hashTree":
            ...
            gaussian_timer_element = [element for element in hashtree.iterchildren() if element.tag == "GaussianRandomTimer"]
            if timer_element:
            ...
            if gaussian_timer_element:      
                timer_delay = self._get_string_prop(gaussian_timer_element[0], 'ConstantTimer.delay')
                random_range = self._get_string_prop(gaussian_timer_element[0], 'RandomTimer.range')
                if timer_delay:
                    timer = {"think-time": "gaussian(" + timer_delay + "ms, " + str(int(float(random_range))) + "ms)" }
                    self.log.debug('Got %s for timer in %s (%s)', timer_delay, element.tag, element.get("testname"))
    ...
    def _get_data_sources(self, element, recursive=False):
        ...
        if hashtree is not None and hashtree.tag == "hashTree":
            ...
            # EDITED - Added support for RandomCSVDataSetConfig
            data_sources_elements = [elem for elem in elements if elem.tag == "CSVDataSet" or elem.tag == "RandomCSVDataSetConfig" ]
            for data_source in data_sources_elements:
                ...
                if data_source is not None:
                    ...
                    # Added loop_prop for RandomCSVDataSetConfig
                    if data_source.tag == "CSVDataSet":
                        loop_prop = self._get_bool_prop(data_source, 'recycle')
                    elif data_source.tag == "RandomCSVDataSetConfig":
                        loop_prop = self._get_bool_prop(data_source, 'rewindOnTheEndOfList')
                    ...
                    random_prop = self._get_bool_prop(data_source, 'randomOrder')
                    if random_prop is None:
                        self.log.warning("Random Order property was not set in %s, using default: False", data_source.tag)
                        random_prop = False
                    data_source_dict["random-order"] = random_prop
            ...
            random_data_sources_elements = [elem for elem in elements if elem.tag == "RandomCSVDataSetConfig"]
            for data_source in random_data_sources_elements:
                self.log.debug("datasource file: %s", data_source.get("testname"))
                if data_source is not None:
                    data_source_dict = {}
                    f_name_prop = self._get_string_prop(data_source, 'filename')
                    ...
                    random_prop = self._get_bool_prop(data_source, 'randomOrder')
                    if random_prop is None:
                        self.log.warning("'Random Order on EOF' property was not set in %s, using default: False",
                                         data_source.tag)
                        random_prop = False
                        data_source_dict["random-order"] = random_prop
